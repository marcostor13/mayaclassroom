...
/**
 * De dónde saca la siembra sus servicios.
 *
 * Es la parte de `INestApplicationContext` que aquí se usa, y nada más. Así la
 * misma siembra vale para el contexto que levanta `bun run seed` y para el
 * `ModuleRef` de la API cuando la plataforma reinicia la demostración desde su
 * pantalla, sin que este fichero tenga que saber cuál de los dos le llamó.
 */
export interface ContenedorDeSiembra {
  get<T>(tipo: Type<T> | string | symbol): T;
}

/** Ajustes de la siembra. */
export interface OpcionesDeSiembra {
  /**
   * Se ejecuta con los servicios ya resueltos y antes de escribir nada.
   *
   * Existe para que reiniciar la demostración pueda borrarla justo ahí: hacerlo
   * antes de llamar a la siembra dejaría la empresa vacía si luego fallara una
   * inyección, que es el fallo más probable de todos y el único que no tiene
   * arreglo automático.
   */
  antesDeSembrar?: () => Promise<void>;
}

/** Lo que quedó sembrado, para contarlo por consola o por la API. */
export interface ResumenDeSiembra {
  ...
  courses: number;
  students: number;
  /** Vídeos que no se pudieron resolver por falta de `PEXELS_API_KEY`. */
  sinVideo: number;
}
 ...
 */
export function anunciarDestino(config: ConfigService): void {
...
export async function sembrarDemostracion(
  app: ContenedorDeSiembra,
  opciones: OpcionesDeSiembra = {},
): Promise<ResumenDeSiembra> {
  ...
  // Aquí está resuelto todo lo que hace falta y no se ha escrito todavía nada.
  // Es el único momento en el que se puede vaciar la empresa sin arriesgarse a
  // dejarla vacía: si algo de arriba no se hubiera podido inyectar, la siembra
  // ya habría fallado y el borrado no habría llegado a ocurrir.
  await opciones.antesDeSembrar?.();
  ...
  return {
    ...
    teachers: [chef.email, panadero.email, chocolatero.email],
